...
test('it renders a row for each tool', function(assert) {
  this.render(hbs`{{workflow-version-tool-details-table testDetails}}`);
  // should be two rows
  assert.equal(this.$('tr.tool-detail-row').length, 2);
});

test('it renders tool names', function(assert) {
  this.render(hbs`{{workflow-version-tool-details-table testDetails}}`);
  assert.equal(this.$('.tool-name').eq(0).text(), 'tool1');
  assert.equal(this.$('.tool-name').eq(1).text(), 'tool2');
});

test('it renders package names and citation links', function(assert) {
  this.render(hbs`{{workflow-version-tool-details-table testDetails}}`);
...
test('it renders docker image names and links', function (assert) {
  this.render(hbs`{{workflow-version-tool-details-table testDetails}}`);
