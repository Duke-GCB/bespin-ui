...
module('Unit | Service | bespin job watcher', function(hooks) {
  setupTest(hooks);
  ...
  test('startWatching wll send add command', function (assert) {
    ...
    let service = this.owner.factoryFor('service:bespin-job-watcher').create({
      'websockets': {
        'socketFor': socketFor
      }
    });
  ...
  test('onMessage with websocket error will be handled', function (assert) {
    let service = this.owner.lookup('service:bespin-job-watcher');
    assert.ok(service);
    var payload = {
      'data': JSON.stringify({
        'status': 'error',
