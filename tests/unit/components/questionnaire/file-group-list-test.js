...
module('Unit | Component | questionnaire/file group list', function(hooks) {
  setupTest(hooks);
  ...
  test('it renders', function(assert) {
    this.owner.factoryFor('component:questionnaire/file-group-list').create({
      fieldName: "SomeField",
      answerChanged: ()=>{}
    });
    this.render();
  ...
  test('it computes answer with field name and files', function (assert) {
    ...
    const fileGroupList = this.owner.factoryFor('component:questionnaire/file-group-list').create({
      groupSize: 2,
  ...
  test('it generates groupTitle based on formatSettings.groupName', function (assert) {
    let fileGroupList = this.owner.factoryFor('component:questionnaire/file-group-list').create({
      formatSettings: {
        groupName: 'Subject'
      },
      fieldName: 'myField',
  ...
  test('it requires fieldName and answerChanged', function(assert) {
    ...
    assert.throws(() => {
      this.owner.factoryFor('component:questionnaire/file-group-list').create({fieldName: "SomeField"});
    });
    assert.throws(() => {
      this.owner.factoryFor('component:questionnaire/file-group-list').create({answerChanged: ()=>{}});
    });
    this.owner.factoryFor('component:questionnaire/file-group-list').create({fieldName: "SomeField", answerChanged: ()=>{}});
  });
});
