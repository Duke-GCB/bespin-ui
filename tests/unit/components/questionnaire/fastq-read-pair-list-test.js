...
module('Unit | Component | questionnaire/fastq read pair list', function(hooks) {
  setupTest(hooks);
  ...
  test('it computes answer with field name and files', function (assert) {
    ...
    const fastqReadPairList = this.owner.factoryFor('component:questionnaire/fastq-read-pair-list').create({
      groupSize: 2,
  ...
  test('it provides feature support message with link to bespin-cli', function(assert) {
    const fastqReadPairList = this.owner.factoryFor('component:questionnaire/fastq-read-pair-list').create({fieldName: "SomeField", answerChanged: ()=>{}});
