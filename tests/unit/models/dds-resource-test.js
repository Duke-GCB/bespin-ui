...
module('Unit | Model | dds resource', function(hooks) {
  setupTest(hooks);

  test('it exists', function(assert) {
    let model = run(() => this.owner.lookup('service:store').createRecord('dds-resource'));
  ...
  test('it generates a CWL File object', function(assert) {
    const ddsFile = run(() => this.owner.lookup('service:store').createRecord('dds-resource', {name: 'myfile.txt'}));
  ...
  test('It works with map', function (assert) {
    run(() => {
      const files = [
        this.owner.lookup('service:store').createRecord('dds-resource', {name: 'file1.txt'}),
        this.owner.lookup('service:store').createRecord('dds-resource', {name: 'file2.txt'}),
        this.owner.lookup('service:store').createRecord('dds-resource', {name: 'file3.txt'})
      ];
  ...
  test('It creates job input files with DDS IDs', function (assert) {
    run(() => {
      ...
      const project = this.owner.lookup('service:store').createRecord('dds-project', { id: projectId });
      const cred =  this.owner.lookup('service:store').createRecord('dds-user-credential', { id: 7 });
      const ddsFile = this.owner.lookup('service:store').createRecord('dds-resource', {
        name: 'file1.txt',
        project: project,
        id: fileId,
        size: 2000,
