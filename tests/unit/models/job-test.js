...
module('Unit | Model | job', function(hooks) {
  ...
  test('it computes job state properties', function(assert) {
    ...
    let job = run(() => this.owner.lookup('service:store').createRecord('job'));
    run(() => {
  ...
  test('it sends actions to the adapter', function(assert) {
    ...
    this.owner.lookup('service:store').set('peekRecord', (modelName) => {
  ...
  test('it contains getLiveUsage', function(assert) {
    ...
    const job = run(() => this.owner.lookup('service:store').createRecord('job', {id: 123}));
    this.owner.lookup('service:store').set('adapterFor', (modelName) => {
      assert.equal(modelName, 'job');
      return {
        getLiveUsage(jobId) {
